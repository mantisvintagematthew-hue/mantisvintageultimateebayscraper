...
def resolve_dates(declared: dict, inferred: dict) -> dict:
    ...
    declared_conf = declared.get("declared_confidence") or 0
    inferred_conf = inferred.get("inferred_confidence") or 0
    ...
    if dy is not None and is_ is not None and ie is not None:
        if is_ <= dy <= ie:
            return {
                ...
                "canonical_confidence": max(declared_conf, inferred_conf),
                "conflict_flag": False,
                "resolution_reason": "declared_within_inferred",
            }

        gap = min(abs(dy - is_), abs(dy - ie))
        return {
            "canonical_year": iy,
            "canonical_start_year": is_,
            "canonical_end_year": ie,
            "canonical_confidence": inferred_conf,
            "conflict_flag": gap >= 3,
